...
class TestPriceCache:
    ...
    def test_remove_bumps_version(self):
        """Removing a ticker must bump the version counter so SSE
        consumers polling `version` can detect the change and push the
        removal to clients."""
        cache = PriceCache()
        cache.update("AAPL", 190.00)
    ...
    def test_remove_nonexistent_does_not_bump_version(self):
        """Removing a ticker that was never cached is a no-op and should
        not bump the version counter."""
        cache = PriceCache()
        v_before = cache.version
        cache.remove("AAPL")
